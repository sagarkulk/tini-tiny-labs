...
function generateExpressions({
	...
	for (let i = 0; i < count; i++) {
		let a = getRandomInt(
			Math.max(LIMITS.NUMBERS.MIN, min),
			Math.min(LIMITS.NUMBERS.MAX, max)
		);
		let b = getRandomInt(
			Math.max(LIMITS.NUMBERS.MIN, min),
			Math.min(LIMITS.NUMBERS.MAX, max)
		...
		switch (type) {
			...
			case "div":
				...
				if (fractionalDiv) {
					...
					answer = Number(
						(a / b).toFixed(
							clamp(divPrecision, LIMITS.PRECISION.MIN, LIMITS.PRECISION.MAX)
						)
					);
				} else {
...
export default function Mathematics() {
	...
	const noneSelected =
		!operations.add && !operations.sub && !operations.mul && !operations.div;
	...
	const score = useMemo(() => {
		...
		const correct = allProblems.filter((p) =>
			isCorrectAnswer(p.userAnswer, p.answer)
		).length;
		return {
			correct,
			total,
	...
	useEffect(() => {
		...
		const allCorrect = allProblems.every((p) =>
			isCorrectAnswer(p.userAnswer, p.answer)
		);
		if (allCorrect) {
			setFinished(true);
	...
	const generateAll = () => {
		...
		const count = clamp(
			Math.floor(Number(numEquations) || 1),
			LIMITS.EQUATIONS.MIN,
			LIMITS.EQUATIONS.MAX
		);
		let min = clamp(
			Number.isFinite(minNumber) ? minNumber : 0,
			LIMITS.NUMBERS.MIN,
			LIMITS.NUMBERS.MAX
		);
		let max = clamp(
			Number.isFinite(maxNumber) ? maxNumber : min + 10,
			LIMITS.NUMBERS.MIN,
			LIMITS.NUMBERS.MAX
		);
		...
		setAddition(
			operations.add ? generateExpressions({ type: "add", ...common }) : []
		);
		setSubtraction(
			operations.sub ? generateExpressions({ type: "sub", ...common }) : []
		);
		setMultiplication(
			operations.mul ? generateExpressions({ type: "mul", ...common }) : []
		);
		setDivision(
			operations.div ? generateExpressions({ type: "div", ...common }) : []
		);
	...
	const ProgressBar = ({ remaining, total }) => {
		const pct = total
			? clamp(Math.round((remaining / total) * 100), 0, 100)
			: 0;
		return (
					...
					{data.map((item, idx) => {
						...
						return (
							<div
								key={idx}
								className={`problem ${
									submitted ? (correct ? "done" : "miss") : ""
								}`}
							>
								{/* VERTICAL EQUATION */}
								<div
									className="equation-vertical"
									aria-label={`${title} problem ${idx + 1}`}
								>
								...
								<div className="answerRow">
									...
									/>
									{submitted && (
										<span className={`mark ${correct ? "ok" : "no"}`}>
											{correct ? "✓" : "✗"}
										</span>
									)}
								</div>
			...
			<div className="statusBar">
				<div className="row" role="status" aria-live="polite">
					{locked ? (
						timedEnabled ? (
							<span className="pill">
								Remaining: {formatVerboseMinSec(remaining)}
							</span>
						) : (
						...
						<span className="pill">
							{`Completed in ${formatTime(elapsed)}${
								timedEnabled && autoSubmitted ? " (time up)" : ""
							}`}
						</span>
					) : timedEnabled ? (
						<span className="pill">
							...
							<input
								...
								onChange={(e) =>
									setDurationMin(
										clamp(
											Math.floor(
												Number(e.target.value) || LIMITS.DURATION.MIN_MINUTES
											),
											LIMITS.DURATION.MIN_MINUTES,
											LIMITS.DURATION.MAX_MINUTES
										)
				...
				<div className="row">
					...
					{(locked || finished) && (
						<button
							className="primaryBtn"
							onClick={startNewRoundAfterFinish}
								...
								<input
									...
									onChange={(e) =>
										setNumEquations(
											clamp(
												Math.floor(
													Number(e.target.value) || LIMITS.EQUATIONS.MIN
												),
												LIMITS.EQUATIONS.MIN,
												LIMITS.EQUATIONS.MAX
											)
