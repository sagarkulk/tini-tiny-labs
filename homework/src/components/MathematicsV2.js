...
function isCorrectAnswer(userAnswer, expected, epsilon = 1e-6) {
  ...
  const ua = Number(userAnswer); if (Number.isNaN(ua)) return false;
...
function generateExpressions({ type, count, min, max, nonNegativeSub, fractionalDiv, divPrecision }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (type) {
      ...
      case "sub": if (nonNegativeSub && a < b) [a, b] = [b, a]; expr = `${a} - ${b}`; answer = a - b; break;
...
export default function Mathematics() {
  ...
  const startCountdown = (totalSec) => {
    stopTimer(); setRemaining(totalSec); setElapsed(0); setAutoSubmitted(false);
    intervalRef.current = setInterval(() => {
      setRemaining(prev => {
        if (prev <= 1) {
          stopTimer(); setFinished(true); setSubmitted(true); setAutoSubmitted(true); setLocked(false);
          return 0;
        }
        return prev - 1;
      });
  ...
  useEffect(() => {
    ...
    if (allCorrect) { setFinished(true); setSubmitted(true); stopTimer(); setLocked(false); }
  ...
  const generateAll = () => {
    ...
    const common = {
      ...
      min, max, nonNegativeSub, fractionalDiv,
    ...
    setSubmitted(false); setFinished(false); setLocked(true);

    if (timedEnabled) {
      const totalSec = clamp(Math.floor(Number(durationMin) || 1) * 60, 30, 60 * 180);
      startCountdown(totalSec);
    } else {
      stopTimer(); setElapsed(0); setRemaining(0); setAutoSubmitted(false);
    }
  };
  ...
  const resetAll = () => {
    ...
    setSubmitted(false); setFinished(false); setLocked(false);
    setElapsed(0); setRemaining(0); setAutoSubmitted(false);
  };
  ...
  const handleSubmit = () => {
    if (submitted) return;
    setSubmitted(true); setFinished(true); stopTimer(); setLocked(false);
  };
  ...
  const totalSeconds = useMemo(
    () => (timedEnabled ? (locked ? remaining + elapsed : durationMin * 60) : 0),
                ...
                <div className="answerRow">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (raw === "") { handleChange(type, idx, ""); return; }
  ...
  return (
    <div className="wrap">
      <style>{`
        ...
        fieldset.group { border:1px solid var(--divider); border-radius:12px; background: var(--header-bg); padding:0; min-inline-size: 0; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08); }
        ...
        .numberInput { width:100%; max-width:100%; padding:10px 12px; border:1px solid var(--divider); border-radius:8px; background: var(--header-bg); color: var(--text-color); box-sizing: border-box; min-width:0; -webkit-appearance: none; appearance: none; }
      ...
      <div className="statusBar">
        <div className="row" role="status" aria-live="polite">
          {locked ? (
            ...
            <span className="pill">
              {timedEnabled ? `Completed in ${formatTime(elapsed)}${autoSubmitted ? " (time up)" : ""}` : `Practice finished`}
            </span>
          ) : timedEnabled ? (
            <span className="pill">
              Duration:
              <input
                ...
                title="Edit minutes (1–180). Applies when you press Start."
                onWheel={(e) => e.target.blur()}
                onChange={(e) => setDurationMin(clamp(Math.floor(Number(e.target.value) || 1), 1, 180))}
              />
              minutes
            </span>
          ) : (
        ...
        {timedEnabled && <ProgressBar remaining={locked ? remaining : durationMin * 60} total={totalSeconds} />}
        ...
        <div className="row">
          ...
          {(locked || finished || allProblems.length > 0) && (
            <button className="startOverBtn" onClick={resetAll} title="Reset to initial state">Reset</button>
          )}
          {(locked || finished) && (
            ...
            <div className="groupGrid">
              ...
              {timedEnabled && (
                <label className="ctrlPair">
                  ...
                  <input
                    type="number" min="1" max="180" className="numberInput"
                    value={durationMin}
                    onWheel={(e) => e.target.blur()}
                    onChange={(e) => setDurationMin(clamp(Math.floor(Number(e.target.value) || 1), 1, 180))}
                  />
              ...
              <label className="ctrlPair">
                ...
                <input
                  ...
                  value={numEquations}
                  onWheel={(e) => e.target.blur()}
                  onChange={(e) => setNumEquations(Math.max(1, Math.min(20, Math.floor(Number(e.target.value) || 1))))}
                />
              </label>
              <label className="ctrlPair">
                <span className="label">Min</span>
                <input
                  type="number" className="numberInput" min="-500" max="500"
                  value={minNumber}
                  onWheel={(e) => e.target.blur()}
                  onChange={(e) => setMinNumber(Math.max(-500, Math.min(500, Number(e.target.value) || 0)))}
                />
              </label>
              <label className="ctrlPair">
                ...
                <input
                  ...
                  value={maxNumber}
                  onWheel={(e) => e.target.blur()}
                  onChange={(e) => setMaxNumber(Math.max(-500, Math.min(500, Number(e.target.value) || 0)))}
                />
              </label>

              <label className="ctrlPair">
                <span className="label">Non-negative subtraction</span>
                <input type="checkbox" className="checkInput" checked={nonNegativeSub}
                  onChange={(e) => setNonNegativeSub(e.target.checked)} />
              </label>

              <label className="ctrlPair">
                <span className="label">Fractional division</span>
                <input type="checkbox" className="checkInput" checked={fractionalDiv}
              ...
              <label className="ctrlPair">
                ...
                <input
                  type="number" min="0" max="4" className="numberInput"
                  value={divPrecision}
                  onWheel={(e) => e.target.blur()}
                  onChange={(e) => setDivPrecision(Math.max(0, Math.min(4, Math.floor(Number(e.target.value) || 0))))}
                  disabled={!fractionalDiv}
                  title={fractionalDiv ? "Set number of decimal places (0–4)" : "Enable fractional division to adjust precision"}
                />
              </label>
            </div>
          </div>
      ...
      {submitted && (
        <div className="summary" role="region" aria-label="Assessment summary" ref={summaryRef} tabIndex={-1}>
          <div className="big">Score: {score.correct} / {score.total} ({score.pct}%)</div>
