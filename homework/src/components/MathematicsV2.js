...
function generateExpressions({ type, count, min, max, nonNegativeSub, fractionalDiv, divPrecision }) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    switch (type) {
      ...
      case "div":
        if (b === 0) b = 1;
        if (fractionalDiv) { expr = `${a} ÷ ${b}`; answer = Number((a / b).toFixed(clamp(divPrecision, LIMITS.PRECISION.MIN, LIMITS.PRECISION.MAX))); }
...
export default function Mathematics() {
  ...
  const generateAll = () => {
    ...
    if (timedEnabled) {
      const totalSec =
        clamp(Math.floor(Number(durationMin) || LIMITS.DURATION.MIN_MINUTES) * 60,
  ...
  const totalSeconds = useMemo(
    () => (timedEnabled ? (locked ? remaining + elapsed : clamp(durationMin, LIMITS.DURATION.MIN_MINUTES, LIMITS.DURATION.MAX_MINUTES) * 60) : 0),
  ...
  return (
    <div className="wrap">
      ...
      <div className="statusBar">
          ...
          {locked ? (
            ...
            <span className="pill">
              ...
              <input
                ...
                onWheel={(e) => e.target.blur()}
                onChange={(e) => setDurationMin(clamp(Math.floor(Number(e.target.value) || LIMITS.DURATION.MIN_MINUTES), LIMITS.DURATION.MIN_MINUTES, LIMITS.DURATION.MAX_MINUTES))}
              />
              minutes
            </span>
          ) : (
        ...
        {timedEnabled && <ProgressBar remaining={locked ? remaining : clamp(durationMin, LIMITS.DURATION.MIN_MINUTES, LIMITS.DURATION.MAX_MINUTES) * 60} total={totalSeconds} />}
            ...
            <div className="groupGrid">
              ...
              <label className="ctrlPair">
                ...
                <input
                  ...
                  value={numEquations}
                  onWheel={(e) => e.target.blur()}
                  onChange={(e) => setNumEquations(clamp(Math.floor(Number(e.target.value) || LIMITS.EQUATIONS.MIN), LIMITS.EQUATIONS.MIN, LIMITS.EQUATIONS.MAX))}
                />
              </label>
              <label className="ctrlPair">
